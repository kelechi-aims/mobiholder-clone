...
import { signupslider } from "../_lib/data";
...
const SignUpSlider = () => {
  ...
  return (
    <div className="bg-[#15171e] pt-12 pb-4 px-13 h-screen flex flex-col justify-between">
      <div className="flex gap-4 justify-between ">
        {signupslider.map((_, index) => (
          <div
            key={index}
            className={`h-1.5 w-24 rounded transition-all ${currentSlider === index ? "bg-[#242ef2]" : "bg-[#242ef2] bg-opacity-30"}`}
          ></div>
        ))}
      </div>
